...
int main(void)
{
	...
	// ###### USING ARRAY AS A POINTER i.e : Value Of xth Elements Of iArray : (iArray_aab + x) AND Address of xth Element Of iArray : (iArray_aab + x) ######
	printf("\n\n");
	printf(" *** USING ARRAY NAME AS A POINTER i.e: value of xth Elements Of iArray_aab : *(iArray_aab + x) AND Address Of iArray_aab : (iArray_aab + x) ***\n\n ");
	...
	printf("*(iArray_aab + 6) = %d \t At Address (iArray_aab + 6) : %p\n", *(iArray_aab + 6), (iArray_aab + 6));
	...
	//ASSIGNING BASE ADDRESS OF INTEGER ARRAY 'iArray_aab' TO INTEGER POINTER 'ptr_iArray_aab'
	//NAME OF ANY ARRAY IS ITS BASE ADDRESS
	ptr_iArray_aab = iArray_aab; //SAME AS ... ptr_iArray_aab = &iArray_aab[0]

	//##### USING POINTER AS ARRAY NAME i.e : Value Of xth Element Of iArray : ptr_iArray_aab[x] AND aDDRESS oF xth Elements Of iArray : &ptr_iArray_aab[x] AND Address Of xth Element Of iArray[x] ####
	printf("\n\n");
	printf(" *** USING POINTER AS ARRAY NAME i.e: Value Of xth Element Of iArray : ptr_iArray_aab[x] AND Address Of iArray_aab : &ptr_iArray_aab[x] ***\n\n");
